...
<body>
	<form action="student/studentAction_edit?pageNum=${pageNum }" method="post" id="myform">
		<input hidden name="stuID" value="${stuID}" />
		<table class="insert-tab" width="70%">	
			<tbody>
				<tr>
					<th>所属班级(必选)：</th>
					<td>
						<select name="clazz.clzID" form="myform">
							<option value="${Clazz.clzID }" selected="selected" >${clazz.clzName }</option>
				...
				<tr>
					...
					<td>
						<select name="stuSex" form="myform">
							<option value="${stuSex }" selected="selected" >${stuSex }</option>
				...
				<tr>
					...
					<td>
						<select name="stuStatus" form="myform">
							<option value="${stuStatus }" selected="selected" >${stuStatus }</option>
